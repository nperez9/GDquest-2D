...
static func fs_find(pattern: String = "*", path: String = "res://") -> Array[String]:
	...
	while path.is_valid_filename():
		...
		if dir.current_is_dir():
			if not is_file and (path == pattern or path.match(pattern)):
				result.push_back(new_path)
			result += fs_find(pattern, new_path)
		elif path == pattern or path.match(pattern):
			result.push_back(new_path)
		path = dir.get_next()
	return result
...
static func precache_icon_image(icon_name: String) -> bool:
	...
	const FALLBACK_NAME := "ObjectDisabled"

	var editor_theme := EditorInterface.get_editor_theme()
	var source_name := icon_name
	if not editor_theme.has_icon(source_name, EDITOR_ICONS):
	...
	if not ResourceLoader.exists(icon_path):
		...
		var icon := editor_theme.get_icon(source_name, EDITOR_ICONS)
