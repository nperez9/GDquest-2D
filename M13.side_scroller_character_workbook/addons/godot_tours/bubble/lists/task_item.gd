...
var _shortcut_events: Array[InputEventKey] = []
...
func _process(_delta: float) -> void:
	...
	_shortcut_events.clear() # Flush for the next frame.


func _input(event: InputEvent) -> void:
	var ke := event as InputEventKey
	if ke and ke.pressed:
		_shortcut_events.push_back(ke)
...
func is_shortcut_pressed(shortcut_name: String) -> bool:
	...
	if not editor_settings.has_shortcut(shortcut_name):
		return false

	for event in _shortcut_events:
		if editor_settings.is_shortcut(shortcut_name, event):
			return true
