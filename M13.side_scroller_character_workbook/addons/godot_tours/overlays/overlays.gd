...
## Returns true if there are any dimmers currently managed by this overlays instance.
func has_dimmers() -> bool:
	return not _dimmer_map.is_empty()
...
func _create_auto_clamp_getter(control: Control) -> Callable:
	...
	while current_node:
		# If the chain of controls is broken, don't clamp to anything. Includes canvas
		# layers and viewports/windows.
		if current_node is not Control:
			break

		# Inspector property editors and sections are clamped to the inspector. This only
		# applies to the main inspector, and includes its sub-inspectors.
		if current_node == main_inspector:
			return main_inspector.get_global_rect

		# Children of dockable nodes are clamped to the dock's container.
		if current_node is EditorDock:
			return func() -> Rect2:
				var dock_container := current_node.get_parent()
