...
class ResourcePreloaderDockDef extends Types.Definition:
	func _init() -> void:
		node_type = "ResourcePreloaderEditor"
		prefetch_references = [
		...
		var custom_script := func(base_node: Node) -> Node:
			...
			for dock_container in dock_locations:
				var dock := dock_container.find_child("ResourcePreloader", false, false)
				if dock:
					return dock
