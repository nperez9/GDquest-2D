...
func is_last_tour(tour: Tour) -> bool:
	...
	# First, check straightforward cases.
	if tour_index < 0:
		return false # Invalid tour.
	if tour_index >= tours.size() - 1:
		return true # Definitely last.

	# Last, check if it's next sibling is locked, which makes this one effectively last.
	var next_tour := tours[tour_index + 1]
	return not next_tour.is_available()


func get_next_tour(tour: Tour) -> Tour:
	var tour_index := tours.find(tour)
...
class Tour:
	...
	var is_locked := false
	## Flag indicating the tour has been completed by the user.
	...
	func is_available() -> bool:
		return not script_path.is_empty() and not is_locked
