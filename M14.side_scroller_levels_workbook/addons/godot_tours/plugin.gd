...
var _supported_versions := {
	min = { major = 4, minor = 7, op = func(a: int, b: int) -> bool: return a <= b },
	max = { major = 4, minor = 7, op = func(a: int, b: int) -> bool: return a >= b },
}
...
## File path for the metadata itself, wherever it is actually found.
var _series_metadata_path: String = ""
...
const PROGRESS_SAVE_FILEPATH := "user://gdtour_progress.save"
const PROGRESS_SAVE_VERSION := 1

## List of completed tour IDs, that we save to disk and restore between
## Godot sessions.
var _completed_tour_ids: Array[String] = []
...
func _enter_tree() -> void:
	...
	var pot_files: Array[String] = [ plugin_path.path_join("tour.gd"), _series_metadata_path ]
	...
	Tour.overlays = _gdtour_overlays
...
func _load_tour_series() -> void:
	...
	for script_path in TOUR_SCRIPT_PATHS:
		...
		# Load before initializing the metadata object, so translations can be applied.
		Utils.load_tour_translation(script_path.get_base_dir(), "")

		_series_metadata = tour_script.new()
		_series_metadata_path = script_path
...
func _update_tour_list() -> void:
	...
	for tour in _series_metadata.tours:
		tour.is_completed = tour.id in _completed_tour_ids
...
func _show_tour_list() -> void:
	...
	# We refresh the tour display each time to reflect any changes to user
	# progress data.
	_update_tour_list()
...
func _start_tour(tour_id: String, skip_intro: bool = false) -> void:
	...
	if not tour_metadata or not tour_metadata.is_available():
		return
	...
	var TourScript: Script = load(tour_metadata.script_path)
	_current_tour = TourScript.new(tour_metadata, skip_intro, tour_is_last)
	_gdtour_ui_root.add_child(_current_tour)

	_current_tour.closed.connect(_gdtour_button.show)
	_current_tour.ended.connect(_on_tour_ended)
...
func _on_tour_ended() -> void:
	var tour_id := _current_tour.tour_metadata.id
	if not tour_id in _completed_tour_ids:
		_completed_tour_ids.push_back(tour_id)

	_save_progress_to_disk()
	_continue_tour_series()


func _continue_tour_series() -> void:
	var next_tour := _series_metadata.get_next_tour(_current_tour.tour_metadata)
	if not next_tour or not next_tour.is_available():
		_show_tour_list()
		return

	_start_tour(next_tour.id, true)


func _load_progress_from_disk() -> void:
	if not FileAccess.file_exists(PROGRESS_SAVE_FILEPATH):
		return

	var file := FileAccess.open(PROGRESS_SAVE_FILEPATH, FileAccess.READ)
	if not file:
		push_error("GDTour: Could not open progress save file: " + PROGRESS_SAVE_FILEPATH)
		return

	var data := str_to_var(file.get_as_text())
	if data is not Dictionary:
		push_error("GDTour: Progress save file content should be a dictionary. Aborting load: " + PROGRESS_SAVE_FILEPATH)
		return

	var version: int = data.get("version", 0)
	if version != PROGRESS_SAVE_VERSION:
		push_error("GDTour: Progress save file version mismatch. Aborting load: " + PROGRESS_SAVE_FILEPATH)
		return

	var completed: Array = data.get("completed", [])
	if completed is not Array:
		push_error(
			"GDTour: Progress save file should contain a dictionary with a 'completed' array. Aborting load: " + PROGRESS_SAVE_FILEPATH
		)
		return
	_completed_tour_ids = completed


func _save_progress_to_disk() -> void:
	var data := {
		version = PROGRESS_SAVE_VERSION,
		completed = _completed_tour_ids,
	}

	var file := FileAccess.open(PROGRESS_SAVE_FILEPATH, FileAccess.WRITE)
	if not file:
		push_error("GDTour: Could not open progress save file to write save data: " + PROGRESS_SAVE_FILEPATH)
		return
...
func _reset_tour(tour_id: String) -> void:
	...
	if was_reset_successful:
		_completed_tour_ids.erase(tour_id)
		_save_progress_to_disk()
		_update_tour_list()
...
func _toggle_debugger_dock_visible() -> void:
	if debugger == null:
		...
		debugger.setup(
			_series_metadata, _gdtour_overlays, _current_tour, debug_mode_toggled
		)
		debugger.tour_start_requested.connect(_start_tour_from_debugger)
		debugger.set_is_debug_mode(is_debug_mode)

	if debugger.is_inside_tree():
		_gdtour_overlays.remove_highlights_from_control(debugger)
		debugger.tour_start_requested.disconnect(_start_tour_from_debugger)
		remove_control_from_docks(debugger)
		debugger.queue_free()
		debugger = null
		if _current_tour == null:
...
func _start_tour_from_debugger(tour_id: String) -> void:
	_start_tour(tour_id)
	if is_instance_valid(debugger):
		debugger.on_tour_started(_current_tour)
