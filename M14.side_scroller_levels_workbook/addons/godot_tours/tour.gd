...
# NOTE: Track statically instead of passing a reference to each tour instance.
static var overlays: Overlays = null
...
var _command_context: CommandContext = CommandContext.TOUR_STEP
var _reset_commands: Array[Command] = []
...
func _init(metadata: GDTourMetadata.Tour, skip_intro: bool = false, is_last: bool = false) -> void:
	name = "Tour"
	tour_metadata = metadata
	tour_is_last = is_last
...
func _build_bookends(skip_intro: bool) -> void:
	var layout_root := EditorInterfaceAccess.get_node(EditorNodePoints.LAYOUT_ROOT)

	# Create a welcome step.

	var tour_reset_func := func() -> void:
		bubble.clear()

	var tour_start: Array[Command] = [
		Command.new(_clean_up_step),
		Command.new(tour_reset_func),
	]
	tour_start += _reset_commands

	if skip_intro:
		var intro_skip_func := func() -> void:
			bubble.visible = false
			await delay_process_frame()
			bubble.visible = true
			next()

		tour_start.push_back(Command.new(intro_skip_func))
	else:
		...
		tour_start.push_back(Command.new(tour_start_func))
		tour_start += _welcome_commands

	steps.push_front(tour_start)

	# Create a finale step.
	...
	tour_end += _finale_commands

	steps.push_back(tour_end)


## Runs all commands for the given [param step] sequentially. Commands can be
## asynchronous and the execution then is halted until the command returns.
func _execute_step(step: Array[Command]) -> void:
	var i := 0
	for current_command in step:
...
func auto_back() -> void:
	queue_command(
		func wait_for_frame_and_step_back() -> void:
			await delay_process_frame()
			back()
	)
...
func auto_next() -> void:
	queue_command(
		func wait_for_frame_and_step_next() -> void:
			await delay_process_frame()
			next()
	)
...
## Changes context for queued commands. Affects which step the commands added via [method queue_command]
## are added to. See [enum CommandContext].
func set_command_context(context: CommandContext) -> void:
	_command_context = context


## Appends a command to the set according to the current command context. See [method set_command_context]
## and [enum CommandContext]. Commands are executed in the order they are added.
##
## When creating tour steps, call [method complete_step] to complete the current step and start creating
## the next one.
func queue_command(callable: Callable, parameters := []) -> void:
	var command := Command.new(callable, parameters)

	match _command_context:
		CommandContext.TOUR_STEP:
			_step_commands.push_back(command)
		CommandContext.TOUR_RESET:
			_reset_commands.push_back(command)
		CommandContext.WELCOME_BOOKEND:
			_welcome_commands.push_back(command)
		CommandContext.FINALE_BOOKEND:
			_finale_commands.push_back(command)
...
func bubble_set_bookend_title(text: String) -> void:
	queue_command(func() -> void: bubble.set_bookend_title(text))


func bubble_add_bookend_text(text: Array[String], style: Bubble.BookendTextStyle) -> void:
	queue_command(func() -> void: bubble.add_bookend_text(text, style))


func bubble_set_bookend_button_text(text: String) -> void:
	queue_command(func() -> void: bubble.set_bookend_button_text(text))
	...
	bubble_add_task(
		...
		func focus_camera_task(task: Task) -> int:
			...
			for i in 4:
				...
				if spatial_viewport_overlay.has_focus() and task.is_shortcut_pressed("spatial_editor/focus_selection"):
...
func context_set_asset_store() -> void:
	context_set("Asset Store")
	...
	queue_command(
		func() -> void:
			...
			if path in EditorInterface.get_open_scenes():
				# NOTE: Reload doesn't guarantee a scene switch; in fact, this would be
				# considered a bug. So we always want to call open afterwards. As of 4.7,
				# these calls aren't supposed to have any deferred logic, so it's safe
				# to call them directly without awaiting anything.
