...
func _update_labels() -> void:
	...
	if tour_metadata.is_completed:
		_steps_label.text = "%d / %d" % [ tour_metadata.step_count, tour_metadata.step_count ]
		_steps_label.modulate = PROGRESS_COLORS[2]
	else:
		_steps_label.text = "0 / %d" % [ tour_metadata.step_count ]
...
func _update_reset_icon() -> void:
	...
	# This icon is used for users to manually erase their saved progress for a
	# given tour. That's why we show it only when a tour was completed.
	_reset_anchor.visible = tour_metadata.is_completed
