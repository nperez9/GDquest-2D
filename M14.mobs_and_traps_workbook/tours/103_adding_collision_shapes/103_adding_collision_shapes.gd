...
const START_SCENE_PATH = "res://creating_tilesets.tscn"
...
func add_step_open_start_scene_conditionally() -> void:
	...
	if start_scene_root == null or start_scene_root.name != ROOT_NODE_NAME:
		highlight_filesystem_paths([START_SCENE_PATH])
		...
		bubble_add_text([
			...
			gtr("In the [b]FileSystem Dock[/b] at the bottom-left, find and [b]double-click[/b] on the scene we will be working with: [b]%s[/b].") % START_SCENE_PATH.get_file(),
		])
		bubble_add_task(
			gtr("Open the scene [b]%s[/b].") % START_SCENE_PATH.get_file(),
			1,
		...
		mouse_move_by_callable(
			...
			get_tree_item_center_by_path.bind(interface.filesystem_tree, START_SCENE_PATH),
		)
		mouse_click()
		mouse_click()
		complete_step()
...
func _build() -> void:
	...
	bubble_set_title("")
	bubble_add_text([bbcode_wrap_font_size("[center][b]Adding Physics to Your Tileset![/b][/center]", 32)])
	bubble_add_text([
	...
	bubble_set_avatar_at(Bubble.AvatarAt.LEFT)
	bubble_add_text(["Let's open the [b]TileSet[/b] editor to add collision shapes to our tiles."])
