...
func _build() -> void:
	...
	bubble_set_title("")
	bubble_add_text([bbcode_wrap_font_size("[center][b]Creating Your First Tileset![/b][/center]", 32)])
	bubble_add_text([
		"[center]In this tour, you'll learn how to create and set up a tileset in Godot.[/center]",
		"[center]You'll create a tileset from scratch, add tiles to it, and use your tileset to draw a simple platform.[/center]",
		"[center]In the next tour, you'll add collision shapes to your tiles to prevent the player from falling through the platform.[/center]",
	])
	...
	bubble_add_text([
		...
		"We don't have any tiles yet, so we cannot draw anything! Open the [b]TileSet editor[/b] so we can start adding tiles to the [b]TileSet[/b].",
	])
	...
	bubble_set_title("Tile sourcees")
	bubble_move_and_anchor(interface.canvas_item_editor, Bubble.At.BOTTOM_CENTER)
	bubble_set_avatar_at(Bubble.AvatarAt.LEFT)
	bubble_add_text([
		"We call the texture containing tiles a [b]Tile Source[/b]. It's a single image containing multiple tiles arranged in a grid.",
		"Our artist prepared tile sourcees for you to use in this module. We'll use this one to create our first tileset:"
	])
	...
	bubble_add_text([
		...
		"[b]Important:[/b] A pop-up will ask you if you want to automatically create tiles. Select [b]No[/b] for now. We'll learn how to create tiles manually first."
	])
	...
	bubble_set_avatar_at(Bubble.AvatarAt.LEFT)
	bubble_add_text([
		"There are two ways to create tiles from an source:",
		...
		"You'll use both methods in this tour. Let's start with the manual one."
	])
	complete_step()
	...
	bubble_add_task(
		...
		func(_task: Task) -> int:
			...
			for child in scene_root.get_children():
				if child is TileMapLayer and child.get_used_cells().size() > 0:
					return 1
			return 0
	)
	complete_step()
	...
	const PATH_TILESET_RESOURCE = "res://creating_tilesets.tres"
